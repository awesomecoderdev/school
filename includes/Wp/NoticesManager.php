...
namespace AwesomeCoder\School\Wp;

use AwesomeCoder\School\Utils\Arrays;
use AwesomeCoder\School\Utils\Functions;
...
class NoticesManager
{
    ...
    public function handleAjax()
    {
        ...
        if (!wp_verify_nonce($nonce, $this->nonce)) {
            status_header(400);
            echo __('Invalid nonce', 'plagiarism');
        } else {
            $notice = $this->notices[$id] ?? null;
    ...
    {
        wp_enqueue_script('plagiarism-admin-notices', $this->script, ['jquery'], SCHOOL_THEME_VERSION, true);
